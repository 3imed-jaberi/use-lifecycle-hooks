...
// #######################################################################
  ...
  function ShouldComponentUpdate ({ content }) {
    ...
    useShouldComponentUpdate(() => {
      setState(ShouldComponentUpdateMsg);
    }, [content]);
...
// #######################################################################
  ...
  describe('use-lifecycle-hooks', () => {
    ...
    test('useShouldComponentUpdate', () => {
      let component;

      act(() => {
        component = mount(<ShouldComponentUpdate />);
        component.setProps({ content: 'someProps' });
      });
      
      expect(component.text()).toBe(ShouldComponentUpdateMsg);
    });
